...
@testset "UMFPACK wrappers" begin
    ...
    @testset "Core functionality for $Tv elements" for Tv in (Float64, ComplexF64)
        ...
        for Ti in Base.uniontypes(SuiteSparse.UMFPACK.UMFITypes)
            ...
            @test A'*x ≈ b
            z = complex.(b)
            x = Base.LinAlg.ldiv!(adjoint(lua), z)
            @test x ≈ float([1:5;])
            @test x === z
            y = similar(x)
            Base.LinAlg.ldiv!(y, adjoint(lua), complex.(b))
            @test y ≈ x
            ...
            x = Base.LinAlg.ldiv!(transpose(lua), complex.(b))
            @test x ≈ float([1:5;])
            y = similar(x)
            Base.LinAlg.ldiv!(y, transpose(lua), complex.(b))
            @test y ≈ x
